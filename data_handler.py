...
class Data_handler:
    ...
    def __init__(self, data_version, data_root, util_root, num_tr_img, \
        ...
        self.tr_loc = np.fromfile(('%s/tr_%d_%d_%d.bin') %
                                  (util_root, num_tr_img, self.half_patch, self.half_range), '<f4').reshape(-1,
                                                                                                            5).astype(
            int)
        ...
        if num_val_img == 0:
            ...
            self.val_loc = np.fromfile(('%s/val_%d_%d_%d.bin') %
                                       (util_root, num_val_img, self.half_patch, self.half_range), '<f4').reshape(-1,
                                                                                                                  5).astype(
                int)
        ...
        self.batch_right = np.zeros(
            (self.batch_size, self.patch_size, self.patch_size + self.disp_range - 1, self.num_channels))
        ...
        self.val_right = np.zeros(
            (num_val_loc, self.patch_size, self.patch_size + self.disp_range - 1, self.num_channels))
        ...
        for i in range(num_val_loc):
            ...
            img_id, loc_type, center_x, center_y, right_center_x = self.val_loc[self.val_perm[i], 0], self.val_loc[
                self.val_perm[i], 1], self.val_loc[self.val_perm[i], 2], self.val_loc[self.val_perm[i], 3], \
                                                                   self.val_loc[self.val_perm[i], 4]
            right_center_y = center_y
            # print(self.rdata[img_id].shape)
            # print(self.val_loc[self.val_perm[i]])

            self.val_left[i] = self.ldata[img_id][(center_y - self.half_patch): (center_y + self.half_patch + 1),
                               (center_x - self.half_patch): (center_x + self.half_patch + 1), :]
            if loc_type == 1:  # horizontal
                self.val_right[i] = self.rdata[img_id][
                ...
                self.val_right[i] = np.transpose(self.rdata[img_id][
    ...
    def next_batch(self):
        ...
        for idx in range(self.batch_size):
            ...
            img_id, loc_type, center_x, center_y, right_center_x = self.tr_loc[self.tr_perm[i]][0], \
                                                                   self.tr_loc[self.tr_perm[i]][1], \
                                                                   self.tr_loc[self.tr_perm[i]][2], \
                                                                   self.tr_loc[self.tr_perm[i]][3], \
                                                                   self.tr_loc[self.tr_perm[i]][4]
            right_center_y = center_y

            if loc_type == 1:  # horizontal
                self.batch_left[idx] = self.ldata[img_id][
                                       (center_y - self.half_patch): (center_y + self.half_patch + 1),
                                       (center_x - self.half_patch): (center_x + self.half_patch + 1), :]
                self.batch_right[idx] = self.rdata[img_id][
                                        right_center_y - self.half_patch: right_center_y + self.half_patch + 1,
                                        right_center_x - self.half_patch - self.half_range: right_center_x + self.half_patch + self.half_range + 1,
                                        :]
            elif loc_type == 2:  # vertical
                self.batch_left[idx] = np.transpose(
                    self.ldata[img_id][(center_y - self.half_patch): (center_y + self.half_patch + 1),
                    (center_x - self.half_patch): (center_x + self.half_patch + 1), :], (1, 0, 2))
                self.batch_right[idx] = np.transpose(self.rdata[img_id][
                                                     right_center_y - self.half_patch - self.half_range: right_center_y + self.half_patch + self.half_range + 1,
                                                     right_center_x - self.half_patch: right_center_x + self.half_patch + 1,
                                                     :], (1, 0, 2))
